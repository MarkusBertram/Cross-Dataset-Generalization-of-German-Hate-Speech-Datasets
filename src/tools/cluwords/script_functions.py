...
def print_results(cluwords_freq, cluwords_docs, path_to_save_results, topics, n_docs):
    ...
    for t in [5, 10, 20]:
        with open('{}/result_topic_{}.txt'.format(path_to_save_results, t), 'w', encoding='utf-8') as f_res:
            f_res.write('Topics {}\n'.format(t))
            f_res.write('Topics:\n')
            topics_t = []
            for topic in topics:
...
def generate_topics(dataset, word_count, path_to_save_model, datasets_path,
    ...
    #np.savetxt('{}/np_c.txt'.format(path_to_save_results), cluwords_tfidf, delimiter=' ')   # X is an array
    #print(cluwords_tfidf)
    ...
    with open('{}/matrix_h.txt'.format(path_to_save_results), 'w') as f:
        h = nmf.components_.transpose()  # matrix H = n x k
        print('Writing matrix H')
        print('W: {} H:{}'.format(w.shape, h.shape))
        for x in range(h.shape[0]):
            for y in range(h.shape[1]):
                f.write('{} '.format(h[x][y]))
            f.write('\n')
        f.close()
        del w
        del h

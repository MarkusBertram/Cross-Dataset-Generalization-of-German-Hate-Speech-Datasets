...
def fetch_dataset(dataset_name, labelled = True, target = False, validation_split = None, seed = None, abusive_ratio = None, truncation_length = None, target_train_split = None):
...
def get_data_loaders(sources, 
    ...
    with FileLock(os.path.expanduser("~/.data.lock")):
        ...
        sampler = BatchSampler(RandomSampler(source_dataset), batch_size=batch_size, drop_last=False)
        train_dataloader = DataLoader(dataset=source_dataset, sampler = sampler, num_workers=num_workers)            
...
def create_model(device, truncation_length):
    ...
    model = labelled_only_model(feature_extractor, task_classifier, output_hidden_states).to(device)  
    
    return model
      
def train_base(config, checkpoint_dir=None):
    seed= 123
    ...
    while True:  # loop over the dataset multiple times
        ...
        for i, data in enumerate(train_loader):
            ...
            class_output = model(input_data=source_features)

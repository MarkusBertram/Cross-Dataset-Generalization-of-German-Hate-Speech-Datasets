...
class CustomBERTModel(nn.Module):
    def __init__(self):
        ...
        ### New layers:
        self.classifier_linear1 = nn.Linear(768, 768)
        self.classifier_linear2 = nn.Linear(768, 768)
        self.classifier_linear3 = nn.Linear(768, 2)
        self.classifier_relu = nn.LeakyReLU()
    ...
    def forward(
        ...
        #x = sequence_output[:,0,:].view(-1,768) ## extract the 1st token's embeddings
        x = outputs[1]
        ...
        logits = self.classifier_softmax(x)
...
if __name__ == '__main__':
    ...
    now = datetime.now()
    results_dir = "./results/"+"2-layer-avg-cross-dataset_performance_"+now.strftime("%Y%m%d-%H%M%S")+"/"
